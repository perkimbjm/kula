...
class SpvExporter implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize
{
    use Exportable;
    ...
    {
        $query = Spv::query()->with(['procurementOfficer', 'consultant']);
    ...
    public function map($spv): array
    {
        ...
        $row = $columns->map(function ($column) use ($spv) {
            ...
            if (in_array($field, $dateFields)) {
                return $spv->$field ? $spv->$field->format('d-m-Y') : '';
            }

            // Handle relationship fields
            if ($field === 'procurementOfficer.name') {
                return $spv->procurementOfficer ? $spv->procurementOfficer->name : '';
            }

            if ($field === 'consultant.name') {
                return $spv->consultant ? $spv->consultant->name : '';
            }

            // Handle numeric fields with formatting
            $numericFields = ['oe', 'bid_value', 'correction_value', 'nego_value', 'payment_value'];
            if (in_array($field, $numericFields)) {
                return $spv->$field ? number_format($spv->$field, 0, ',', '.') : '';
            }
