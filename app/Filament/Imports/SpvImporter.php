...
class SpvImporter implements ToCollection, WithHeadingRow
{

    public function collection(Collection $rows)
    {
        Log::info('SpvImporter: Starting import process', ['total_rows' => $rows->count()]);

        foreach ($rows as $index => $row) {
            $rowNumber = $index + 2; // +2 because index starts from 0 and we have header row

            try {
                Log::info("SpvImporter: Processing row {$rowNumber}", ['row_data' => $row->toArray()]);

                // Map column headers to database fields
                $mappedRow = $this->mapRowData($row);

                Log::info("SpvImporter: Mapped data", ['mapped_data' => $mappedRow]);

                // Validate required fields
                if (empty($mappedRow['contract_number']) || empty($mappedRow['program'])) {
                    Log::warning("SpvImporter: Skipping row {$rowNumber} - missing required fields");
                    continue;
                }
                ...
                if (!empty($cleanedRow['procurement_officer_id'])) {
                    ...
                    if (!$procurementOfficer) {
                        Log::warning("SpvImporter: ProcurementOfficer ID {$cleanedRow['procurement_officer_id']} not found for row {$rowNumber}");
                ...
                if (!empty($cleanedRow['consultant_id'])) {
                    ...
                    if (!$consultant) {
                        Log::warning("SpvImporter: Consultant ID {$cleanedRow['consultant_id']} not found for row {$rowNumber}");
                        unset($cleanedRow['consultant_id']);
                    }
                }

                Log::info("SpvImporter: Final data for row {$rowNumber}", ['final_data' => $cleanedRow]);

                // Create or update Spv (without ID field, let auto-increment handle it)
                $spv = Spv::updateOrCreate(
                    ['contract_number' => $cleanedRow['contract_number']],
                    $cleanedRow
                );

                Log::info("SpvImporter: Successfully processed row {$rowNumber}", ['spv_id' => $spv->id]);

            } catch (\Exception $e) {
                Log::error("SpvImporter: Error processing row {$rowNumber}: " . $e->getMessage());
                Log::error("SpvImporter: Stack trace: " . $e->getTraceAsString());
        ...
        Log::info('SpvImporter: Import process completed');
    }
